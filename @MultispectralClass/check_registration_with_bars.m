...
if 0
    load([folder '/output_images'],'original','unregistered','registered_cp','registered_cp_corr','imf_before','imf_cp_corr')
else
    
    % use Matlab registration app
    load([folder '/final_images'],'unregistered_trimmed','registered_cp_corr_trimmed')
    
    original = registered_cp_corr_trimmed;
    registered_cp_corr = unregistered_trimmed;
    ...
    switch gca
        ...
        case graph
            ...
            if (cursor(1) >= 1 && cursor(1)<=size(im1,2) &&...
                ...
                xlabel(sprintf('X position (Y=%d/%d), R=%.4f',cursor(2),size(profyline1,1),profxcorrcoef(1,2)))
                ylabel('L^*','Rotation',0)
                ...
                xlabel('L^*')
                ylabel(sprintf('Y position (X=%d/%d), R=%.4f',cursor(1),size(profxline1,2),profycorrcoef(1,2)))
