%%
classdef ColorMatchingFunctionClass 
    %%
    properties
        x_bar
        y_bar
    ...
    %%
    methods
        function obj = ColorMatchingFunctionClass 

            obj.x_bar = SpectrumClass(obj.cmf(:,1)',obj.cmf(:,2)');
            obj.y_bar = SpectrumClass(obj.cmf(:,1)',obj.cmf(:,3)');
            obj.z_bar = SpectrumClass(obj.cmf(:,1)',obj.cmf(:,4)');
        end
        
        function XYZxyz = spec2XYZ (obj,spec)
            wl_selected = ismember(spec.wavelength,obj.cmf(:,1));
            amp2 = spec.amplitude(wl_selected);
            wl_selected_no = nnz(wl_selected);
            X = amp2 * obj.x_bar.amplitude' / wl_selected_no;
            Y = amp2 * obj.y_bar.amplitude' / wl_selected_no;
            Z = amp2 * obj.z_bar.amplitude' / wl_selected_no;
            x = X/(X+Y+Z);
            y = Y/(X+Y+Z);
            z = Z/(X+Y+Z);
            X+Y+Z
